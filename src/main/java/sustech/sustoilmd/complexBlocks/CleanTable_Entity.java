...
import sustech.sustoilmd.ImplementedInventory;
import sustech.sustoilmd.ModEntityTypes;
...
public class CleanTable_Entity extends BlockEntity implements ExtendedScreenHandlerFactory, ImplementedInventory {
    ...
    private final int cookTimeTotal = 200; // 总处理时间
    ...
    private static void craftItem(CleanTable_Entity entity) {
        ...
        if (output.isEmpty()) {
            ...
            int num = (int)(Math.random()*100) + 1;//生成一个[1，100]之间的随机数字
            if (num<=50){
        ...
        if (oil.isOf(ModFluids.OIL_BUCKET)) {
            oil.decrement(1);
            entity.setStack(OIL_SLOT, new ItemStack(Items.BUCKET)); // 替换为空桶
        }
    }
    ...
    public Text getDisplayName() {
        return Text.translatable("container.sustoilmd.clean_table");
    }
    ...
    public static final int OIL_SLOT = 0;    // 石油桶槽位
